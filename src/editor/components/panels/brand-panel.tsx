...
const COLOR_FIELDS: Array<{ key: keyof BrandColors; label: string }> = [
  ...
  { key: "accentSoft", label: "لون التمييز الخفيف" },
  { key: "text", label: "النص" },
  { key: "muted", label: "النص الثانوي" },
...
function BrandSettingsEditor({ settings, onChange }: { settings: BrandSettings; onChange: (settings: BrandSettings) => void }) {
  return (
    <div className="space-y-3 rounded-lg border border-stone-200 bg-stone-50 p-3">
      ...
      <div className="grid grid-cols-2 gap-2">{COLOR_FIELDS.map((field) => <div key={field.key}><Label htmlFor={`brand-${field.key}`}>{field.label}</Label><Input id={`brand-${field.key}`} type="color" value={settings.colors[field.key]} onChange={(event) => onChange({ ...settings, colors: { ...settings.colors, [field.key]: event.target.value } })} /></div>)}</div>
      ...
      <div><Label htmlFor="brand-radius">استدارة البطاقات</Label><Input id="brand-radius" type="number" min={0} max={240} value={settings.radius} onChange={(event) => onChange({ ...settings, radius: Math.max(0, Math.min(240, Number(event.target.value))) })} /></div>
    </div>
  );
}
...
export function BrandPanel() {
  ...
  return (
    <div className="space-y-4 p-4">
      <div><h3 className="font-black">هويات جاهزة</h3><p className="mt-1 text-xs leading-5 text-stone-500">هويات مضمّنة أو مخصصة، محفوظة محلياً على جهازك.</p></div>
      <p className="text-sm text-brand-muted">اختيار هوية يحدّث ألوان السلسلة وخطوطها. استخدم التراجع لإلغاء التغيير.</p>
      <div className="grid gap-3 sm:grid-cols-2">{[...BUILT_IN_BRAND_KITS, ...customKits].map((kit) => <BrandKitCard key={kit.id} kit={kit} active={document.brandKitId === kit.id} onApply={() => apply(kit)} onDelete={customKits.some((custom) => custom.id === kit.id) ? async () => { await deleteLocalBrandKit(kit.id); setCustomKits((current) => current.filter((item) => item.id !== kit.id)); } : undefined} />)}</div>
      <BrandSettingsEditor settings={document.brand} onChange={updateSettings} />
      <div className="rounded-lg border border-stone-200 p-3"><Label>اسم الهوية المحلية</Label><Input value={kitName} onChange={(event) => setKitName(event.target.value)} /><Button type="button" variant="secondary" className="mt-2 w-full" onClick={async () => { const kit = await createLocalBrandKit(kitName, "هوية محلية مخصصة", document.brand); setCustomKits((current) => [kit, ...current]); apply(kit); }}><Plus /> حفظ الهوية</Button></div>
    </div>
  );
}
